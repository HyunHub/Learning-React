...
function App() { // APP 이라는 Component를 만들어 줌 - 함수형 Component, 밑 코드는 HTML이 아니고 JSX(javaScript의 확장 문법)라고 부름
  const name = 'React'; // const - 한 번 설정하면 재설정 불가, let 선언 후 유동적 변할 수 있을 때 사용 for 문,, 둘 다 같은 블록 내 중복 선언 불가능
/*   return ( // <>는 Fragment 생략된 거,, 여러 요소가 무조건 부모 요소 하나에 감싸져 있어야 해서 씀. div를 쓰기 싫을 때 
 <> 
    {name === 'React' ? (
    <h1>Hello {name} !!</h1>
    ) : (
